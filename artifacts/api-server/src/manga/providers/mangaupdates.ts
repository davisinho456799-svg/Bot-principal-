...
function normalizeLastUpdated(
  value: MangaUpdatesSeriesPayload["last_updated"],
): string | null {
  if (typeof value === "string") return value || null;
  if (!value) return null;
  if (value.as_rfc3339) return value.as_rfc3339;
  if (value.as_string) return value.as_string;
  if (value.timestamp == null) return null;
...
function toSeries(payload: MangaUpdatesSeriesPayload): MangaUpdatesSeries | null {
  ...
  return {
    ...
    lastUpdated: normalizeLastUpdated(payload.last_updated),
  };
}
