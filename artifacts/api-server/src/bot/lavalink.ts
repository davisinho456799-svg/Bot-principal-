...
export function initLavalink(client: Client): any | null {
  ...
  manager=new Riffy(client as any,[{name:process.env["LAVALINK_NAME"]??"railway-lavalink",host,port:Number(process.env["LAVALINK_PORT"]??2333),password,secure:process.env["LAVALINK_SECURE"]==="true"}],{send:(p:any)=>{const g=client.guilds.cache.get(p.d.guild_id);if(g)g.shard.send(p);},defaultSearchPlatform:process.env["LAVALINK_SEARCH_PLATFORM"]??"ytmsearch",restVersion:"v4"} as any);
  ...
  client.once("ready",()=>manager.init(client.user!.id)); return manager;
}
