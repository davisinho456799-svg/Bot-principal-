...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption((option) => option.setName("musica").setDescription("Nome, link da música ou playlist").setRequired(true));
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (!manager) {
    await interaction.reply({ content: "❌ O Lavalink não está configurado neste ambiente.", ephemeral: true });
    return;
  }
