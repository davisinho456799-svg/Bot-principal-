...
async function buildMangaEmbed(r: MangaDexResult): Promise<EmbedBuilder> {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(synopsis.slice(0, 4096))
...
export async function execute(
  ...
  if (autocompleteMatch) {
    ...
    try {
      ...
      logger.error({ err, id }, "Erro ao buscar mangá por ID em /manga (autocomplete)");
  ...
  try {
    ...
    collector?.on("collect", async (sel: StringSelectMenuInteraction) => {
      ...
      try {
        ...
        logger.error({ err }, "Erro ao buscar mangá selecionado em /manga (select)");
