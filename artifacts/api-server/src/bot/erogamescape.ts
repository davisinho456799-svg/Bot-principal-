...
/**
 * Busca a data de última atualização (最終更新日) de um jogo no Erogamescape.
 * Retorna um timestamp Unix (ms) ou null se não encontrado.
 */
export async function getErogamescapeLastUpdated(gameId: string): Promise<number | null> {
  try {
    const url = `${BASE}/game.php?game=${gameId}`;
    const html = await fetchHtml(url);

    // Tenta capturar 最終更新日 (última data de atualização)
    const m =
      /最終更新日[\s\S]{0,200}?(\d{4}[-\/年]\d{1,2}[-\/月]\d{1,2})/i.exec(html) ??
      /最終更新[\s\S]{0,200}?(\d{4}[-\/年]\d{1,2}[-\/月]\d{1,2})/i.exec(html);

    if (!m) return null;

    const raw = m[1].replace(/[年月\/]/g, "-").replace(/-$/, "");
