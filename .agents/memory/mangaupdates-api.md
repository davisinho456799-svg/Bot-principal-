...
**How to apply:** Cache the session token only in memory, refresh it after HTTP 401, prefer a positive `latest_chapter`, and otherwise extract the greatest `N Chapters` value from the series status. Normalize `last_updated` because the API returns a timestamp object, not always a string. Treat missing/zero values as no data.