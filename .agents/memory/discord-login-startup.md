...
O token do Discord deve ser escolhido entre variáveis não vazias, com espaços externos removidos, e o tempo de espera do Gateway deve ser configurável por ambiente, mantendo um limite padrão generoso para cold starts. A conexão deve ser supervisionada com novas tentativas e nunca derrubar a API por indisponibilidade temporária do Discord. O status “service live” do provedor HTTP não confirma que o bot recebeu `ClientReady`.
...
**How to apply:** Ao alterar a inicialização do bot, preserve o diagnóstico sem expor o token, valide o token via REST antes do Gateway, aceite `DISCORD_LOGIN_TIMEOUT_MS` para redes lentas, diferencie falha de credencial de atraso de conexão nos logs, destrua clientes que expiraram e use backoff para tentar novamente.